...
class Logs():
    ...
    def get(self, *args):
        ...
        if method == "text":
            ...
            level = request.args.get('level', default=self.ascanner.logger.levelname, type=str)
            limit = request.args.get('limit', default=None, type=str)

            logs = self.ascanner.logger.memory.filter(level=level, limit=limit)

            fakefile = StringIO()

            for log_entry in list(logs.keys()):
                fakefile.write("%s\n" % self.ascanner.logger.memory.dict[log_entry]["fmsg"])
