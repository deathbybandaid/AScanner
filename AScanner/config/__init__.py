...
from AScanner.tools import isint, isfloat, is_arithmetic
...
class Config():
    ...
    def core_setup(self, script_dir):
        ...
        AScanner_web_dir = pathlib.Path(script_dir).joinpath('AScanner_web')
        www_dir = pathlib.Path(AScanner_web_dir).joinpath('www_dir')
        ...
        self.internal["paths"] = {
                                    ...
                                    "AScanner_web_dir": AScanner_web_dir,
                                    "www_dir": www_dir,
                                    "www_templates_dir": pathlib.Path(AScanner_web_dir).joinpath('templates'),
        ...
        # Web Server Conf
        self.read_json_config(pathlib.Path(self.internal["paths"]["AScanner_web_dir"]).joinpath("web_ui_conf.json"))
    ...
    def config_verification(self):
        ...
        if self.dict["main"]["cache_dir"]:
            ...
            if not pathlib.Path(self.dict["main"]["cache_dir"]).is_dir():
                raise AScanner.exceptions.ConfigurationError("Invalid Cache Directory. Exiting...")
